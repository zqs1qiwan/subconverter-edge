...
function parseStatusLine(line: string): SubscriptionInfo | undefined {
  try {
    ...
    for (const part of parts) {
      // Expires 可能在 TOT 段里用 💡 分隔
      const expMatch = part.match(/Expires:(\d{4}-\d{2}-\d{2})/);
      if (expMatch) result.expire = Math.floor(new Date(expMatch[1]).getTime() / 1000);
      // 提取数值+单位
      const m = part.match(/:([\d.]+)\s*(B|KB|MB|GB|TB)/i);
      if (!m) continue;
      const val = parseSize(m[1], m[2]);
      if (part.includes('TOT') || part.includes('TOTAL')) result.total = val;
      else if (part.includes('Expires')) continue; // 已处理
      else if (result.upload === 0) result.upload = val;  // 第一段是 upload
      else if (result.download === 0) result.download = val;  // 第二段是 download
    }
    return result;
  } catch {
    return undefined;
